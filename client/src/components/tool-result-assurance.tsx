...
export function ToolResultAssurance({ toolId }: { toolId?: string }) {
  const consultationHref = toolId
    ? `#/consultation?tool=${encodeURIComponent(toolId)}&cta=result`
    : "#/consultation";
  return (
    <aside className="mt-6 rounded-lg border bg-muted/30 p-4" aria-label="Privacy and consultation information" data-testid="tool-result-assurance">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        ...
        <Link href={consultationHref} className="text-sm font-medium underline" data-testid="result-consultation-cta">Need help with document workflows? Book a consultation</Link>
      </div>
    </aside>
  );
}
