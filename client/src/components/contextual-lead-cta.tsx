...
export interface ContextualLeadCtaProps {
  category: ToolCategory;
  toolSlug: string;
  onConsultationRequest?: (context: LeadContext) => void;
  contactHref?: string;
  className?: string;
}
...
export function ContextualLeadCta({ category, toolSlug, onConsultationRequest, contactHref = "/contact", className }: ContextualLeadCtaProps) {
  ...
  const context: LeadContext = { sourceTool: toolSlug, sourceCta: `${category}-result-cta` };
  function handleConsultation() {
    void trackPublicEvent("cta_clicked", {
      ...
      cta_placement: context.sourceCta,
    });
    onConsultationRequest?.(context);
  }
  return (
    <aside className={className} aria-label="Harmony Digital Consults consultation invitation">
      <h2>{content.heading}</h2>
      <p>{content.body}</p>
      {onConsultationRequest ? (
        <button type="button" onClick={handleConsultation}>Discuss a project</button>
      ) : (
        <a href={contactHref} onClick={handleConsultation}>Discuss a project</a>
      )}
      <a href={contactHref}>Contact Harmony Digital Consults</a>
    </aside>
  );
}
